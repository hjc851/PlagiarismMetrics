...
fun main() {
    ...
    var test = InstanceFilter.filter(data, Predicate { !trainIds.contains(it.stringValue(0)) || baseIds.contains(it.stringValue(0)) })
    ...
    val attributes = (0 until train.numAttributes()).map { train.attribute(it).name() }
